"""
KBD101 K-Cube Brushless DC Motor Controller - Kinesis Backend

Uses Thorlabs Kinesis .NET DLLs via pythonnet.
For high-speed stages like DDSM100, DDS220, DDS300, DDS600.
"""
...
class KBD101Controller(MotorController):
    """
    KBD101 K-Cube Brushless DC Motor Controller.
    
    Supports high-speed stages like DDSM100, DDS220, DDS300, DDS600.
    Note: Brushless stages require phasing on first use.
    """
    ...
    def connect(self) -> bool:
        """Connect to the KBD101 controller."""
        ...
        try:
            ...
            self._device = KCubeBrushlessMotor.CreateKCubeBrushlessMotor(serial_str)
            ...
            raise ConnectionError(f"Failed to connect to KBD101 {self.serial_number}: {e}")
    ...
    def get_velocity_params(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                "max_velocity": to_float(vel_params.MaxVelocity),
                "acceleration": to_float(vel_params.Acceleration),
                "min_velocity": to_float(vel_params.MinVelocity) if hasattr(vel_params, 'MinVelocity') else 0.0,
            }
        except Exception as e:
            print(f"Failed to get velocity params: {e}")
            return {"max_velocity": 0, "acceleration": 0}
    ...
    def get_stage_info(self) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Dict with stage info or None:
                - part_number: Stage model (e.g., "DDS100")
    ...
    # -------------------------------------------------------------------------
    # Brushless-specific methods
    # -------------------------------------------------------------------------
    
    def start_phasing(self) -> bool:
        """
        Start motor phasing procedure.
        
        Required for brushless motors on first use or after motor replacement.
        """
        if not self._device:
            return False
        
        try:
            # Note: Actual phasing API may differ - check Kinesis docs
            if hasattr(self._device, 'SetPhaseParameters'):
                self._device.SetPhaseParameters(True)
            return True
        except Exception as e:
